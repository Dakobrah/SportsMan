...
export async function defenseTotals(
  db: Database,
  filters: ReportFilters,
): Promise<DefenseTotals> {
  const where = snapWhere(defensive(filters), ["kind IN ('RUN', 'PASS')"]);
  ...
  return toDomain<DefenseTotals>(row ?? {});
}
...
export async function defenseByPlayer(
  db: Database,
  filters: ReportFilters,
): Promise<DefenderLine[]> {
  const where = snapWhere(
    defensive(filters),
    ["s.kind IN ('RUN', 'PASS')", 's.primary_player_id IS NOT NULL'],
    's.',
  );
  return toDomainAll<DefenderLine>(
    await db.all(
      `SELECT p.id AS player_id, p.number, p.first_name, p.last_name, p.position,
              COUNT(*)                                                AS tackles,
       ...
       FROM snaps s JOIN players p ON p.id = s.primary_player_id
       WHERE ${where.sql}
       GROUP BY p.id
       ORDER BY tackles DESC, sacks DESC`,
      where.params,
    ),
  );
}
...
export async function assistsByPlayer(
  db: Database,
  filters: ReportFilters,
): Promise<AssistLine[]> {
  const where = snapWhere(defensive(filters), [], 's.');
  return toDomainAll<AssistLine>(
    await db.all(
      `SELECT p.id AS player_id, p.number, p.first_name, p.last_name,
       ...
       FROM defense_assists a
       JOIN snaps s   ON s.id = a.snap_id
       JOIN players p ON p.id = a.player_id
       WHERE ${where.sql}
       GROUP BY p.id
       ORDER BY tackle_assists DESC, sack_assists DESC`,
      where.params,
    ),
  );
}
